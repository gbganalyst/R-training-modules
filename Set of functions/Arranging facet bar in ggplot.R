...
ggplot(iris_gathered, aes(x = reorder(Species, value), y = value, fill = Species)) +
  geom_bar(stat = 'identity', show.legend = F) +
  facet_wrap(~ metric)

# After

ggplot(iris_gathered, aes(x = reorder_within(Species, value, metric), y = value, fill = Species)) +
  geom_bar(stat = 'identity', show.legend = F) +
  scale_x_reordered() +
